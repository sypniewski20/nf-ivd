...
process VEP_GERMLINE_SNV {
    ...
    script:
        def genome  = (fasta.name =~ /(?i)GRCh38|hg38|Homo_sapiens_assembly38/) ? "GRCh38" : "GRCh37"
        def clinvar = params.clinvar ? "--custom file=${params.clinvar},short_name=ClinVar,format=vcf,type=exact,coords=0,fields=CLNSIG%CLNREVSTAT%CLNDN%MC%CLNDISDB%CLNDISDBINC" : ""
        def dbNSFP  = params.dbnsfp ? "--plugin dbNSFP,${params.dbnsfp},SIFT_score,SIFT_pred,Polyphen2_HVAR_score,Polyphen2_HVAR_pred,MutationTaster_score,MutationTaster_pred,MetaRNN_score,MetaRNN_pred,REVEL_score,BayesDel_addAF_score,BayesDel_addAF_pred,BayesDel_noAF_score,BayesDel_noAF_pred,ClinPred_score,ClinPred_pred,AlphaMissense_score,AlphaMissense_pred,CADD_raw,CADD_phred,GERP++_RS,phyloP100way_vertebrate,gnomAD4.1_joint_NFE_AF,gnomAD4.1_joint_AF" : ""
        def spliceai = (params.seq_type != 'WES' && spliceai_vcf.name != 'NO_FILE_VCF') ? "--custom file=${spliceai_vcf},short_name=SpliceAI,format=vcf,type=overlap,coords=0,fields=ALLELE%SYMBOL%DS_AG%DS_AL%DS_DG%DS_DL" : ""
        """
        vep \
            --cache \
